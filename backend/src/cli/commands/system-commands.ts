...
export class SystemCommands {
  ...
  async getSystemStatus(detailed: boolean = false): Promise<SystemStatus> {
    ...
    try {
      const [dbStatus, bedrockStatus, storageStatus, evalStatus] = await Promise.allSettled([
    ...
    try {
      ...
      const config: SystemConfiguration = {
        limits: {
          ...
          currentStudents,
          maxSessionDuration: parseInt(process.env.MAX_SESSION_DURATION || '120'),
        ...
        models: {
          primary: process.env.PRIMARY_MODEL || 'anthropic.claude-3-sonnet-20240229-v1:0',
          fallback: process.env.FALLBACK_MODEL || 'anthropic.claude-3-haiku-20240307-v1:0'
        },
        endpoints: {
          api: process.env.API_ENDPOINT || 'https://api.school-tutor.com',
          dashboard: process.env.DASHBOARD_URL || 'https://dashboard.school-tutor.com'
        }
      };
  ...
  async viewLogs(service?: string, lines: number = 50): Promise<void> {
    ...
    try {
      const logGroups = service ? [this.getLogGroupName(service)] : this.getAllLogGroups();
  ...
  async backupData(): Promise<void> {
    ...
    try {
      ...
      console.log(`🔗 Backup location: s3://school-tutor-backups/${backupName}`);
  ...
  private async checkDatabaseHealth(): Promise<boolean> {
    try {
      const studentsTable = process.env.STUDENTS_TABLE_NAME || 'school-tutor-students';
  ...
  private getStatusFromResult(result: PromiseSettledResult<boolean>): 'healthy' | 'warning' | 'critical' {
  ...
  private determineOverallStatus(services: SystemStatus['services']): 'healthy' | 'warning' | 'critical' {
  ...
  private async getSystemConfiguration(): Promise<SystemStatus['configuration']> {
    const currentStudents = await this.getCurrentStudentCount();
  ...
  async displaySystemStatus(status: SystemStatus): Promise<void> {
    ...
    console.log('\n🖥️  System Status:');
    console.log('═'.repeat(60));
    console.log(`Overall Status: ${statusIcon[status.status]} ${status.status.toUpperCase()}`);
    ...
    Object.entries(status.services).forEach(([service, serviceStatus]) => {
      console.log(`   ${statusIcon[serviceStatus]} ${service}: ${serviceStatus}`);
    });
    
    console.log('\n📊 Metrics:');
    console.log(`   👥 Active Students: ${status.metrics.activeStudents}/${status.configuration.maxStudents}`);
    ...
    console.log(`   👥 Student Limit: ${status.configuration.currentStudents}/${status.configuration.maxStudents}`);
  ...
  async displayConfiguration(config: SystemConfiguration): Promise<void> {
    ...
    console.log('\n📏 Limits:');
    console.log(`   👥 Students: ${config.limits.currentStudents}/${config.limits.maxStudents}`);
    console.log(`   ⏱️  Max Session Duration: ${config.limits.maxSessionDuration} minutes`);
    console.log(`   📊 Max Daily Requests: ${config.limits.maxDailyRequests}`);
    
    console.log('\n🎛️  Features:');
    console.log(`   🧠 Adaptive Learning: ${config.features.adaptiveLearning ? '✅' : '❌'}`);
    console.log(`   📈 Progress Tracking: ${config.features.progressTracking ? '✅' : '❌'}`);
    console.log(`   🔍 Evaluation: ${config.features.evaluation ? '✅' : '❌'}`);
