...
export class ProgressCommands {
  ...
  async viewProgress(
    ...
    try {
      ...
      const subjectMetrics = this.calculateSubjectMetrics(
        progressData,
        subjects,
      );
  ...
  async generateScorecard(
    ...
    try {
      ...
      const scorecard: ScorecardData = {
        ...
        subjectScores: this.generateSubjectScores(
          progressData,
          student.subjects || [],
        ),
  ...
  private async getProgressData(
    ...
    const command = new QueryCommand({
      ...
      KeyConditionExpression:
        "studentId = :studentId AND #timestamp BETWEEN :start AND :end",
  ...
  private calculateOverallMetrics(
    ...
    const totalEngagement = progressData.reduce(
  ...
  private calculateSubjectMetrics(
    ...
    const subjectData = progressData.reduce(
      (acc, item) => {
    ...
    Object.entries(subjectData).forEach(
      ([subject, data]: [string, ProgressDataItem[]]) => {
        if (subjects && !subjects.includes(subject)) return;

        const completedTopics = new Set(
          data.map((d: ProgressDataItem) => d.topic).filter(Boolean),
        ).size;
        const totalScore = data.reduce(
          (sum: number, d: ProgressDataItem) => sum + (d.score || 0),
          0,
        );
        const totalTime = data.reduce(
          (sum: number, d: ProgressDataItem) => sum + (d.timeSpent || 0),
          0,
        );
        const validScores = data.filter(
          (d: ProgressDataItem) => (d.score || 0) > 0,
        );

        subjectMap[subject] = {
          completionRate: Math.round(
            (data.filter((d: ProgressDataItem) => d.status === "completed")
              .length /
              data.length) *
              100,
          ),
  ...
  private analyzeLearningPattern(
    progressData: any[],
  ): AnalyticsData["analytics"]["learningPattern"] {
    ...
    const bestDay =
      Object.entries(dayMap).sort(([, a], [, b]) => b - a)[0]?.[0] || "Monday";
  ...
  private analyzeSkillProgression(
    progressData: any[],
  ): AnalyticsData["analytics"]["skillProgression"] {
    ...
    const recommendations = improvementAreas.map(
      (area) =>
  ...
  private analyzeEngagement(
    progressData: any[],
  ): AnalyticsData["analytics"]["engagement"] {
    const sessions = progressData.filter((p) => p.timeSpent > 0);
    ...
    const questionsAsked = interactions.filter(
      (i) => i.interactionType === "question",
    ).length;
    const hintsUsed = interactions.filter(
      (i) => i.interactionType === "hint",
    ).length;
    ...
    subjects.forEach((subject) => {
      ...
      const avgScore =
        subjectData.length > 0
          ? subjectData.reduce((sum, d) => sum + (d.score || 0), 0) /
            subjectData.length
          : 0;
      ...
      const completedTopics = new Set(
        subjectData.map((d) => d.topic).filter(Boolean),
      ).size;
  ...
  private generateRecommendations(analytics: AnalyticsData): string[] {
    ...
    recommendations.push(
      "Continue practicing strengths while working on improvement areas",
    );
  ...
  private generateNextGoals(analytics: AnalyticsData, student: any): string[] {
    ...
    const improvementAreas =
      analytics.analytics.skillProgression.improvementAreas;
  ...
  private generateSubjectFeedback(
    subject: string,
    score: number,
    progress: number,
  ): string {
  ...
  async displayProgress(progress: ProgressData): Promise<void> {
    ...
    console.log(
      `   🎯 Engagement Score: ${progress.overall.engagementScore}/10`,
    );
    console.log(
      `   📚 Skills Improvement: ${progress.overall.skillsImprovement}%`,
    );
    ...
    if (progress.recentAchievements.length > 0) {
      ...
      progress.recentAchievements.forEach((achievement) => {
        console.log(
          `   🥇 ${achievement.achievement} (${achievement.subject})`,
        );
        console.log(
  ...
  async displayAnalytics(analytics: AnalyticsData): Promise<void> {
    ...
    console.log("\n⏰ Learning Patterns:");
    console.log(
      `   🕐 Peak Hours: ${analytics.analytics.learningPattern.peakHours.join(", ")}`,
    );
    console.log(
      `   ⏱️  Preferred Duration: ${analytics.analytics.learningPattern.preferredDuration} minutes`,
    );
    console.log(
      `   📅 Best Day: ${analytics.analytics.learningPattern.bestPerformingDay}`,
    );

    console.log("\n📈 Skill Progression:");
    if (analytics.analytics.skillProgression.strengths.length > 0) {
      console.log(
        `   💪 Strengths: ${analytics.analytics.skillProgression.strengths.join(", ")}`,
      );
    }
    if (analytics.analytics.skillProgression.improvementAreas.length > 0) {
      console.log(
        `   🎯 Improvement Areas: ${analytics.analytics.skillProgression.improvementAreas.join(", ")}`,
      );
    }
    ...
    console.log("\n🎯 Engagement Metrics:");
    console.log(
      `   ⏰ Average Session: ${analytics.analytics.engagement.averageSessionLength} minutes`,
    );
    console.log(
      `   ❓ Questions Asked: ${analytics.analytics.engagement.questionsAsked}`,
    );
    console.log(
      `   💡 Hints Used: ${analytics.analytics.engagement.hintsUsed}`,
    );
    console.log(
      `   📊 Interaction Rate: ${analytics.analytics.engagement.interactionRate}/10`,
    );
  ...
  async displayScorecard(scorecard: ScorecardData): Promise<void> {
    ...
    console.log(
      `   ⏰ Total Time: ${scorecard.summary.totalTimeSpent} minutes`,
    );
    console.log(
      `   📚 Sessions Completed: ${scorecard.summary.sessionsCompleted}`,
    );
  ...
  static async viewProgress(studentId: string, options: any): Promise<void> {
    ...
    try {
      ...
      const subjects = options.subjects
        ...
        : undefined;
      const progressData = await commands.viewProgress(
        studentId,
        period,
        subjects,
      );
