...
/**
 * Simple logger for structured output
 */
interface Logger {
...
export class EvaluationCommands {
  ...
  constructor() {
    ...
    this.evaluationTable =
      process.env.EVALUATION_TABLE_NAME || "school-tutor-evaluation";
    this.metricsTable =
      process.env.METRICS_TABLE_NAME || "school-tutor-metrics";
  ...
  async runEvaluation(request: EvaluationRequest): Promise<EvaluationResult> {
    ...
    // Use structured logging for evaluation startup
    ...
    try {
      ...
      this.logger.error("Evaluation failed", { evaluationId, error });
  ...
  async launchDashboard(): Promise<DashboardInfo> {
    try {
      ...
      const dashboardUrl =
        ...
        "http://localhost:3000/evaluation-dashboard";
  ...
  private async executeEvaluationFrameworks(
    request: EvaluationRequest,
  ): Promise<any> {
    const results: any = {};
  ...
  private async runRagasEvaluation(
    request: EvaluationRequest,
    ...
    if (
      request.metrics.includes("relevance") ||
      request.metrics.includes("all")
    ) {
      results.relevance = Math.random() * 0.3 + 0.7; // 0.7-1.0
    }

    if (
      request.metrics.includes("groundedness") ||
      request.metrics.includes("all")
    ) {
    ...
    if (
      request.metrics.includes("context_recall") ||
      request.metrics.includes("all")
    ) {
    ...
    if (
      request.metrics.includes("context_precision") ||
      request.metrics.includes("all")
    ) {
  ...
  private async runDeepEvalEvaluation(
    request: EvaluationRequest,
  ): Promise<Record<string, number>> {
    ...
    if (
      request.metrics.includes("factual_consistency") ||
      request.metrics.includes("all")
    ) {
    ...
    if (
      request.metrics.includes("answer_relevancy") ||
      request.metrics.includes("all")
    ) {
    ...
    if (
      request.metrics.includes("conceptual_similarity") ||
      request.metrics.includes("all")
    ) {
  ...
  private async runTruLensEvaluation(
    request: EvaluationRequest,
  ): Promise<Record<string, number>> {
    ...
    if (
      request.metrics.includes("context_relevance") ||
      request.metrics.includes("all")
    ) {
    ...
    if (
      request.metrics.includes("answer_relevance") ||
      request.metrics.includes("all")
    ) {
  ...
  private calculateSummary(results: any): EvaluationResult["summary"] {
    ...
    Object.entries(results).forEach(
      ([framework, frameworkResults]: [string, any]) => {
        if (framework === "overall") return;

        Object.entries(frameworkResults).forEach(
          ([metric, score]: [string, any]) => {
            if (typeof score === "number") {
              if (score < 0.6) {
                criticalIssues.push(
                  `${framework}: ${metric} score is critically low (${Math.round(score * 100)}%)`,
                );
              } else if (score < 0.75) {
                recommendations.push(
                  `Improve ${metric} in ${framework} framework`,
                );
              }
            }
          },
    ...
    if (overall.score < 0.8) {
      recommendations.push(
        "Consider reviewing and improving AI model responses",
      );
    ...
    if (criticalIssues.length === 0 && overall.score >= 0.85) {
      recommendations.push(
        "Excellent performance! Continue monitoring and maintaining quality",
      );
    }
  ...
  private async getRecentEvaluations(
    ...
    try {
      ...
      const result = await this.dynamodb.send(command);
      return (
        result.Items?.map((item) => unmarshall(item) as EvaluationResult) || []
      );
    } catch (error) {
  ...
  ): EvaluationMetrics["systemMetrics"] {
    const completedEvaluations = evaluations.filter(
      (e) => e.status === "completed",
    );
    const passedEvaluations = completedEvaluations.filter(
      (e) => e.summary?.passed,
    );
    ...
    return {
      ...
      passRate:
        completedEvaluations.length > 0
          ? Math.round(
              (passedEvaluations.length / completedEvaluations.length) * 100,
            )
          : 0,
  ...
  private async getFrameworkMetrics(): Promise<
    EvaluationMetrics["frameworkMetrics"]
  > {
  ...
  async displayEvaluationResult(result: EvaluationResult): Promise<void> {
    ...
    if (result.results) {
      ...
      if (result.results.overall) {
        console.log("\n🎯 Overall Performance:");
        console.log(
          `   Score: ${Math.round(result.results.overall.score * 100)}%`,
        );
        console.log(`   Grade: ${result.results.overall.grade}`);
        console.log(
          `   Status: ${result.results.overall.passed ? "✅ PASSED" : "❌ FAILED"}`,
        );
      }
    }
    ...
    if (result.summary) {
      console.log("\n📝 Summary:");
      console.log(
        `   Overall Score: ${Math.round(result.summary.overallScore * 100)}%`,
      );
      console.log(
        `   Result: ${result.summary.passed ? "✅ PASSED" : "❌ FAILED"}`,
      );
  ...
  async displayEvaluationMetrics(metrics: EvaluationMetrics): Promise<void> {
    ...
    console.log("\n📈 System Performance:");
    console.log(
      `   🔢 Total Evaluations: ${metrics.systemMetrics.totalEvaluations}`,
    );
    console.log(`   ✅ Pass Rate: ${metrics.systemMetrics.passRate}%`);
    console.log(
      `   📊 Average Score: ${Math.round(metrics.systemMetrics.averageScore * 100)}%`,
    );
    ...
    console.log(
      `   ${trendIcon[metrics.systemMetrics.trendDirection]} Trend: ${metrics.systemMetrics.trendDirection}`,
    );
    ...
    Object.entries(metrics.frameworkMetrics).forEach(([framework, data]) => {
      ...
      console.log(
        `   ${statusIcon} ${framework.toUpperCase()}: ${data.available ? "Available" : "Unavailable"}`,
      );
      if (data.available) {
        console.log(
          `      Average Score: ${Math.round(data.averageScore * 100)}%`,
        );
      }
    });
    ...
    if (metrics.recentEvaluations.length > 0) {
      ...
      metrics.recentEvaluations.slice(0, 5).forEach((evaluation, index) => {
        console.log(
          `   ${index + 1}. ${evaluation.evaluationId.substring(0, 8)}... - ${Math.round(evaluation.score * 100)}% (${evaluation.status})`,
        );
        console.log(
  ...
  async displayDashboardInfo(info: DashboardInfo): Promise<void> {
    ...
    console.log(
      "   • Use filters to focus on specific metrics or time periods",
    );
    ...
    try {
      const request: EvaluationRequest = {
        ...
        students: options.students
          ? options.students.split(",").map((s: string) => s.trim())
          : undefined,
        frameworks: options.frameworks
          ? options.frameworks.split(",").map((f: string) => f.trim())
          : ["all"],
